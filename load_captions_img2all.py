...
k_8 = os.path.join(base_path, "mixed_coco14_coco14_Xception_glove_concatenate_dense512", "k-8")

result_files_for_beams = {"k-1": k_1, "k-2": k_2, "k-5": k_5, "k-8": k_8}
epoch_list = list()
beam_list = list()
img_id_list = list()
bleu_1_list = list()
    ...
    for model_name in result_files_for_beam:
        ...
        if model_name.endswith(".json") and (not model_name.startswith('.')):
            ...
            for img_id in img_to_eval.keys():
                ...
                epoch = model_name.split("-")[-3]
                epoch_list.append(epoch)
                beam_list.append(beam)
                img_id = img_id.split("/")[-1].replace(".jpg", '')
...
cap = pd.DataFrame(
    {"epoch": epoch_list, "beam": beam_list, "image_id": img_id_list, "Bleu_1": bleu_1_list, "Bleu_2": bleu_2_list,
     "Bleu_3": bleu_3_list,
...
print(cap)
cap.to_csv("img2all.csv")

overall = pd.DataFrame(
    {"epoch": overall_epoch_list, "beam": overall_beam_list, "Bleu_1": overall_bleu_1_list,
...
overall.to_csv("overall_img2all.csv")
